...
При запуске программа покажет текущий размер клетки и предложит ввести новый.
Нажми `Enter`, чтобы оставить прежнее значение. Новый размер сохраняется в
`config.json` рядом с программой.
